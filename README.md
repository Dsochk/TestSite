...
## Что было изменено в PoC (CVE-2025-55182)

Я привёл PoC в рабочий и более универсальный вид, без привязки к локальному стенду:
- удалил локальный сервер на `localhost:3004`, чтобы запросы шли сразу на целевой URL;
- добавил обработку аргументов командной строки (цель и команда);
- сделал `application/x-www-form-urlencoded` вместо multipart, так надёжнее проходит парсинг;
- оставил локальный тест опционально (`--local`), чтобы не мешал основной атаке.
